...
/*   Updated: 2023/04/20 11:30:52 by jomirand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* void	fork_init(t_rules *rules)
{
	int	i;

	i = 0;
	rules->fork = (t_fork *)malloc(sizeof(t_fork) * rules->num_philos);
	while (i < rules->num_philos)
	{
		rules->fork[i].id = i + 1;
		if (pthread_mutex_init(&rules->fork[i].lock, NULL) != 0)
...
t_philo	*create_philos(t_rules *rules, pthread_mutex_t *forks)
{
	...
	while(i < rules->num_philos)
	{
		...
		rules->ph[i].l_fork = &forks[i];
		rules->ph[i].r_fork = &forks[(i + 1) % rules->num_philos];
...
void	create_threads(t_rules *rules)
{
	...
	while(i < rules->num_philos)
	{
		if(pthread_create(&rules->ph[i].philo, NULL, &simulation, &rules->ph[i]) != 0)
			error_message("Could't create thread!\n");
		//usleep(100);
		i++;
	}
...
void	join_threads(t_rules *rules, pthread_mutex_t *forks)
{
	...
	i = 0;
	printf("vou retornar1\n");
	if(pthread_join(rules->monitor, NULL) != 0)
	{
		printf("vou dar join1\n");
		error_message("Could't join thread1!\n");
	}
	while(i < rules->num_philos)
	{
		if(pthread_join(rules->ph[i].philo, NULL) != 0)
			error_message("Could't join thread2!\n");
		//usleep(100);
		i++;
	}
	i = 0;
	if(pthread_mutex_destroy(&rules->print) != 0)
		error_message("Could't join thread5!\n");
	while(i < rules->num_philos)
	{
		pthread_mutex_unlock(&forks[i]);
		//printf("dei unlock ao %d\n", i);
		ret = pthread_mutex_destroy(&forks[i]);
		/* printf("%d\n", ret);
			error_message("Could't join thread3!\n"); */
		i++;
	}
}