...
pthread_mutex_t	*create_forks(t_rules *rules)
{
	...
	if (pthread_mutex_init(&rules->print, NULL) != 0)
		error_message("failed to init mutex!\n");
	if (pthread_mutex_init(&rules->verifs, NULL) != 0)
		error_message("failed to init mutex!\n");
	return (forks);
}
...
void	create_threads(t_rules *r)
{
	...
	{
		if (pthread_create(&r->ph[i].philo, NULL, &simulation, &r->ph[i]) != 0)
			error_message("Couldn't create thread!\n");
		usleep(100);
		i++;
	}
	if (pthread_create(&r->monitor, NULL, &monitoring, r) != 0)
		error_message("Couldn't create thread!\n");
}
